...
class MISLNet(FwdContinualBackbone):
    NAME = 'mislnet'
    arch = {
        256: {
    ...
    def __init__(
        ...
        **args,
        ...
        self.init_weights()
        self.constrain_conv()
        lightning_checkpoint_path = 'epoch=97-step=183750-v_loss=0.0654-v_acc=0.9763.ckpt'
        ...
        print('MISLNet sucessfully loaded from', lightning_checkpoint_path)
    ...
    def forward(self, x, returnt="logits"):
        ...
        if self.num_classes > 0:
            out = self.output(dense_out)
        else:
            out = dense_out
        if returnt == "all":
            return out, None, self.dense
